...
if [ -z "$WEATHERAPI_KEY" ]; then
    echo "❌ 錯誤：WEATHERAPI_KEY 未設定，請在 ~/.openclaw/.env 中設定。"
    exit 1
fi
...
# 检查是否成功获取天气
if [ -z "$WEATHER_JSON" ] || [ "$WEATHER_JSON" = "null" ] || echo "$WEATHER_JSON" | jq -e '.error' >/dev/null 2>&1; then
    # 重试一次：用 URL 编码方式
    ENCODED_CITY=$(python3 -c "import urllib.parse; print(urllib.parse.quote('${CITY}'))" 2>/dev/null || echo "${CITY}")
    if [ "$ENCODED_CITY" != "${CITY}" ]; then
        WEATHER_JSON=$(curl -s "https://api.weatherapi.com/v1/forecast.json?key=${WEATHERAPI_KEY}&q=${ENCODED_CITY}&days=${DAYS}&lang=zh&aqi=yes")
    fi
fi
...
# 如果沒有 SUNSETHUE_KEY，就跳過霞光預測
if [ -z "$SUNSETHUE_KEY" ]; then
    echo -e "\nℹ️ 提示：未偵測到 SUNSETHUE_KEY，跳過霞光品質預測。"
    exit 0
fi
...
SUNRISE_QUALITY=$(echo "$SUNRISE_JSON" | jq -r '.data.quality | if . == null then 0 else (. * 100 | floor) end')
...
SUNSET_QUALITY=$(echo "$SUNSET_JSON" | jq -r '.data.quality | if . == null then 0 else (. * 100 | floor) end')
